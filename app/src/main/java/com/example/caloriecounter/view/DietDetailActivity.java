...
public class DietDetailActivity extends BaseActivity {
    ...
    private void loadDiet(String id) {
        repo.fetchDietById(id, new SupabaseRepository.DietCallback() {
            @Override public void onSuccess(Diet diet) {
                renderDiet(diet);
                String accent = com.example.caloriecounter.utils.ThemeUtils.getAccent(DietDetailActivity.this);
                com.example.caloriecounter.utils.ThemeUtils.applyAccent(binding.llContent, accent);
            }
    ...
    private void addSection(String title, String content) {
        ...
        t.setTextColor(Color.parseColor("#6C63FF"));
        t.setTag("accent");
        t.setPadding(0, 16, 0, 8);
        binding.llContent.addView(t);
        TextView c = new TextView(this);
        c.setText(content);
        c.setTextSize(15);
        c.setTextColor(0xFF333333);
        c.setPadding(8, 0, 8, 16);
        binding.llContent.addView(c);
        addDivider();
    }
    ...
    private void addListSection(String title, String json) {
        ...
        try {
            ...
            err.setText(json);
            err.setTextSize(15);
            err.setPadding(16, 0, 8, 16);
            binding.llContent.addView(err);
        }
        addDivider();
    }
